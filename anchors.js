...
$(window).ready(function()
{
    ...
    {
        let e = $(this);
        let idPrefix = e.prop("tagName").toLowerCase() === 'h1' ? 'section' : 'article';
        let id = idPrefix + '_' + e.text().toLowerCase().replace(/\W+/g, '_');
        e.attr('id', id);
        e.attr('data-frag', id);
    ...
    $("a.header-anchor").click(function(event)
    {
        ...
        top.window.location.hash = "#" + window.location.href.split('/').slice(-1)[0].split('.')[0] + "&" + $(this).parent().attr('id');
